...
from . import ws_base
from .ws_base import USER_AGENT
    ...
    async def _get_wbi_key(self):
        ...
        try:
            async with self._session.get(
                ...
                if res.status != 200:
                    logger.warning(
                        "WbiSigner failed to get wbi key: status=%d %s",
                        res.status,
                        res.reason,
                    )
                    return ""
...
class WebClient(ws_base.WebSocketClientBase):
    ...
    def __init__(
        ...
        *,
        ...
        """原始cookie字符串，API请求时直接通过header携带（不依赖cookie_jar行为）"""
    ...
    async def _init_uid(self):
        ...
        try:
            _response, data = await self._api_request("GET", UID_INIT_URL)
            if not data:
                return False
    ...
    async def _api_request(
        ...
        try:
            headers = {"User-Agent": USER_AGENT}
            if self._cookie_str:
                # 直接通过header携带cookie，确保登录态生效
                # （cookie_jar对无域名cookie的行为依aiohttp版本而异，不可靠）
    ...
    def _handle_command(self, command: dict) -> None:
        ...
        try:
            ...
            if cmd in ("DANMU_MSG", "DANMU_MSG_MIRROR"):
                ...
                dm_message.is_mirror = cmd == "DANMU_MSG_MIRROR"
                ...
                # 礼物消息（protobuf编码，一次可能包含多个礼物）
                data = command.get("data", {})
                for gift_message in web_models.GiftMessage.batch_from_command_v2(data):
                    self._message_queue.put_nowait(
                        message.GiftMessage.from_web_message(
                            gift_message, self._room_id, command
                        )
                    )
            ...
            elif cmd in ("INTERACT_WORD_V2", "INTERACT_WORD"):
                # 互动消息
                data = command.get("data", {})
                interact_message = web_models.InteractWordV2Message.from_command(data)

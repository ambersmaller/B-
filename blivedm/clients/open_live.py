...
class OpenLiveClient(ws_base.WebSocketClientBase):
    ...
    async def _start_game(self):
        try:
            async with self._request_open_live(
                START_URL, {"code": self._room_owner_auth_code, "app_id": self._app_id}
            ) as res:
                if res.status != 200:
                    logger.warning(
                        "_start_game() failed, status=%d, reason=%s",
                        res.status,
                        res.reason,
                    )
                    return False
                ...
                if data["code"] != 0:
                    logger.warning(
                        "_start_game() failed, code=%d, message=%s, request_id=%s",
                        data["code"],
                        data["message"],
                        data["request_id"],
                    )
    ...
    async def _end_game(self):
        ...
        try:
            async with self._request_open_live(
                END_URL, {"app_id": self._app_id, "game_id": self._game_id}
            ) as res:
                if res.status != 200:
                    logger.warning(
                        "room=%d _end_game() failed, status=%d, reason=%s",
                        self._room_id,
                        res.status,
                        res.reason,
                    )
                ...
                if code != 0:
                    ...
                    logger.warning(
                        "room=%d _end_game() failed, code=%d, message=%s, request_id=%s",
                        self._room_id,
                        code,
                        data["message"],
                        data["request_id"],
    ...
    async def _send_game_heartbeat(self):
        ...
        try:
            ...
            async with self._request_open_live(
                HEARTBEAT_URL, {"game_id": game_id}
            ) as res:
                if res.status != 200:
                    logger.warning(
                        "room=%d _send_game_heartbeat() failed, status=%d, reason=%s",
                        self._room_id,
                        res.status,
                        res.reason,
                    )
                ...
                if code != 0:
                    logger.warning(
                        "room=%d _send_game_heartbeat() failed, code=%d, message=%s, request_id=%s",
                        self._room_id,
                        code,
                        data["message"],
                        data["request_id"],
    ...
    def _handle_command(self, command: dict) -> None:
        ...
        if (
            ...
            and command["data"]["game_id"] == self._game_id
        ):
            # 服务器主动停止推送，可能是心跳超时，需要重新开启项目
            logger.warning(
                "room=%d game end by server, game_id=%s", self._room_id, self._game_id
            )
        ...
        try:
            ...
            # 非开放平台消息不处理
            if not cmd.startswith("LIVE_OPEN_PLATFORM_"):
                return
            ...
            if cmd in ("LIVE_OPEN_PLATFORM_DM", "LIVE_OPEN_PLATFORM_DM_MIRROR"):
                # 弹幕消息
                dm_message = open_models.DanmakuMessage.from_command(data)
                dm_message.is_mirror = cmd == "LIVE_OPEN_PLATFORM_DM_MIRROR"
                bili_message = message.DanmakuMessage.from_open_live_message(
                    dm_message, command
                )

            elif cmd == "LIVE_OPEN_PLATFORM_SEND_GIFT":
                # 礼物消息
                gift_message = open_models.GiftMessage.from_command(data)
                bili_message = message.GiftMessage.from_open_live_message(
                    gift_message, command
                )

            elif cmd == "LIVE_OPEN_PLATFORM_SUPER_CHAT":
                # 醒目留言消息
                sc_message = open_models.SuperChatMessage.from_command(data)
                bili_message = message.SuperChatMessage.from_open_live_message(
                    sc_message, command
                )

            elif cmd == "LIVE_OPEN_PLATFORM_LIKE":
                # 点赞消息
                like_message = open_models.LikeMessage.from_command(data)
                bili_message = message.LikeMessage.from_open_live_message(
                    like_message, command
                )

            elif cmd == "LIVE_OPEN_PLATFORM_LIVE_ROOM_ENTER":
                # 进入直播间消息(不知道为什么收不到)
                enter_room_message = open_models.RoomEnterMessage.from_command(data)
                bili_message = message.EnterRoomMessage.from_open_live_message(
                    enter_room_message, command
                )

            elif cmd == "LIVE_OPEN_PLATFORM_GUARD":
                # 上舰消息
                guard_buy_message = open_models.GuardBuyMessage.from_command(data)
                bili_message = message.GuardBuyMessage.from_open_live_message(
                    guard_buy_message, command
                )
